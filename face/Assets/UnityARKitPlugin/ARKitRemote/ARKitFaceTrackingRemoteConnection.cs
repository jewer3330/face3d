...
namespace UnityEngine.XR.iOS
{
	public class ARKitFaceTrackingRemoteConnection : MonoBehaviour
	{
		...
		void Start () {
			...
			editorConnection.Register (ConnectionMessageIds.screenCaptureLandmarksId, ReceiveRemoteScreenLandmarks);
		...
		void ReceiveRemoteScreenYTex(MessageEventArgs mea)
		{
			...
			var arVideo = UnityARVideo.Instance;
			if (arVideo) {
		...
		void ReceiveRemoteScreenLandmarks(MessageEventArgs mea)
		{
			var landmarksRaw = mea.data.Deserialize<Dictionary<string, serializableVector2>>();
			var landmarks = landmarksRaw.ToDictionary(pair => pair.Key, pair => (Vector2) pair.Value);
			UnityARSessionNativeInterface.SetStaticLandmark(landmarks);
			UnityARSessionNativeInterface.RunEditorLandmarkUpdateCallbacks();
		}
