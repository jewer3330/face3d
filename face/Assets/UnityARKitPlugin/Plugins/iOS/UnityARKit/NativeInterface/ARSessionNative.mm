...
extern "C"{
    typedef void (*UNITY_AR_FRAME_IMAGE_CALLBACK)(void* textureY ,unsigned long numYBytes,void* textureCbCr,unsigned long numUVBytes,UnityARMatrix4x4 matrix);
}
static UNITY_AR_FRAME_IMAGE_CALLBACK s_ImageCallback;

extern "C" void unity_SetImageCallback(UNITY_AR_FRAME_IMAGE_CALLBACK frameImageCallback)
{
    s_ImageCallback = frameImageCallback;
}
...
//        NSLog(@"didUpdateFrame:   now: %f, frame.timestamp: %lf, anchors: %lu, capturedDepthDataTimestamp: %lf",
//              [[NSDate date]timeIntervalSince1970], frame.timestamp,
//              frame.anchors.count, frame.capturedDepthDataTimestamp);
//    }
//    else{
//        NSLog(@"didUpdateFrame:   now: %f, frame.timestamp: %lf, anchors: %lu, anchor[0](%s): %@ capturedDepthDataTimestamp: %lf",
//              [[NSDate date]timeIntervalSince1970], frame.timestamp,
//              frame.anchors.count, ((ARFaceAnchor *)frame.anchors[0]).isTracked ? "Tracked" : "NotTracked", ((ARFaceAnchor *)frame.anchors[0]),
//              frame.capturedDepthDataTimestamp);
//    }
//    if(!isnan(frame.capturedDepthDataTimestamp)){
//        // [2865.94 0.00 0.00 | 0.00 2865.94 0.00 | 1922.31 1087.55 1.00], pixelSize:0.001 mm, distortionCenter:{1922.31,1087.55}, ref:{3840x2160}
//        auto calib = frame.capturedDepthData.cameraCalibrationData;
//        NSLog(@"capturedDepthData: Quality: %s, Accuracy: %s, Filtered: %s, depthDataType: 0x%X, size: %ld*%ld@%ldbpp",
//               frame.capturedDepthData.depthDataQuality == AVDepthDataQualityLow ? "low" : "high",
//               frame.capturedDepthData.depthDataAccuracy == AVDepthDataAccuracyAbsolute ? "absolute" : "relative",
//               frame.capturedDepthData.depthDataFiltered ? "true" : "false",
//               frame.capturedDepthData.depthDataType,
////              frame.capturedDepthData.cameraCalibrationData.intrinsicMatrix
//               CVPixelBufferGetWidth(frame.capturedDepthData.depthDataMap),
//               CVPixelBufferGetHeight(frame.capturedDepthData.depthDataMap),
//               CVPixelBufferGetBytesPerRow(frame.capturedDepthData.depthDataMap)*8/CVPixelBufferGetWidth(frame.capturedDepthData.depthDataMap)
//              );
//    }
//    debugSaveDepthAuto(frame);
//    if(frame.capturedDepthDataTimestamp > 0){
//        NSString *name = [NSString stringWithFormat:@"%lf", frame.timestamp];
//        debugSavePointCloud(frame, name);
//    }
    ...
    if (_frameCallback != NULL)
    {
        ...
        // anchors
        unityARCamera.anchors = (__bridge_retained void*)frame.anchors;
        unityARCamera.anchorsSize = frame.anchors.count;
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            CFRelease(unityARCamera.anchors);
        });
    }
    ...
    if(s_ImageCallback != NULL)
    {
        CVPixelBufferLockBaseAddress(pixelBuffer, kCVPixelBufferLock_ReadOnly);


        unsigned long numPBytes = CVPixelBufferGetBytesPerRowOfPlane(pixelBuffer, 0) * CVPixelBufferGetHeightOfPlane(pixelBuffer,0);
        Byte * pYPixelBytes = new Byte[numPBytes];
        void* baseAddress = CVPixelBufferGetBaseAddressOfPlane(pixelBuffer,0);
        memcpy(pYPixelBytes, baseAddress, numPBytes);


        unsigned long numUVBytes = CVPixelBufferGetBytesPerRowOfPlane(pixelBuffer, 1) * CVPixelBufferGetHeightOfPlane(pixelBuffer,1);
        baseAddress = CVPixelBufferGetBaseAddressOfPlane(pixelBuffer,1);
        Byte * pUVPixelBytes = new Byte[numUVBytes];
        memcpy(pUVPixelBytes, baseAddress, numUVBytes);


        CVPixelBufferUnlockBaseAddress(pixelBuffer, kCVPixelBufferLock_ReadOnly);

        s_ImageCallback(pYPixelBytes,numPBytes,pUVPixelBytes,numUVBytes,displayTransform);

        delete[]pYPixelBytes;
        delete[]pUVPixelBytes;
    }
...
void printAnchorInfo(NSString *name, ARSession *session, UnityARSession *unitySession, NSArray<ARAnchor*> *anchors)
{
    NSLog(@"%@ now: %f, frame.timestamp: %lf, anchors: %lu", name, [[NSDate date] timeIntervalSince1970], session.currentFrame.timestamp, anchors.count);
}
...
extern "C" UnityARFaceAnchorData GetUnityARFaceAnchorData(intptr_t ptrAnchors, uint32_t index)
{
#if ARKIT_USES_FACETRACKING
    auto anchors = (__bridge NSArray<__kindof ARAnchor *> *) (void*)ptrAnchors;
    if([anchors[index] isKindOfClass:[ARFaceAnchor class]]){
        UnityARFaceAnchorData anchorData;
        UnityARFaceAnchorDataFromARFaceAnchorPtr(anchorData, (ARFaceAnchor*)anchors[index]);
        return anchorData;
    }
    else
    {
        return UnityARFaceAnchorData{};
    }
#endif
}

extern "C" void SaveFrameImages(const void* session)
{
    UnityARSession* nativeSession = (__bridge UnityARSession*)session;
    ARFrame *frame = nativeSession->_session.currentFrame;
    if(isnan(frame.capturedDepthDataTimestamp)){
        NSLog(@"SaveFrameImages error: no capturedDepthData this frame");
        return;
    }

    saveCVPixelBuffer(frame.capturedDepthData.depthDataMap, @"capturedDepthData.raw");
    saveCVPixelBuffer(frame.capturedImage, @"capturedImage.raw");
}
