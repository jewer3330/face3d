Shader "Clx/character/silkpbralpha"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (VertexOutput i) : SV_Target
			{
				...
				pbrspec += lerp(pbrspec, skyColor , fresnel1);
				pbrspec += pow(NdotH, 50) * pbrspec * 50 * col * roughness;
				half3 skyreflection = skyColor * fresnel * metalness * 0.5;//校正天空盒反射
				//计算高光偏移
				...
				//fixed3 anisospecblend = anisospec * hsvcol1;
				...
				//fragColor.rgb = (specularity + KD * col ) * (shadow * lightingModel + i.ambient);
				fragColor.a = mainTex.a;
				//fragColor = TexHSV(fragColor, _Hue1, _Saturation1, _Brightness1);
