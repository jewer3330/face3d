// Packages/com.unity.probuilder/Editor/EditorCore/ObjExporter.cs
    ...
    static class ObjExporter
    {
        ...
        static string WriteObjContents(string name, IEnumerable<Model> models, Dictionary<Material, string> materialMap, ObjOptions options)
        {
            ...
            foreach (Model model in models)
            {
                ...
                if (textures0 != null && textures0.Length == 0) textures0 = null;
        ...
        static int AppendArrayVec2(StringBuilder sb, Vector2[] array, string prefix, bool mergeCoincident, out Dictionary<int, int> coincidentIndexMap)
        {
            if (array == null)
            {
                coincidentIndexMap = null;
                return 0;
            }

            coincidentIndexMap = new Dictionary<int, int>();

            Dictionary<Vector2, int> common = new Dictionary<Vector2, int>();
            int index = 0;
        ...
        static int AppendArrayVec3(StringBuilder sb, Vector3[] array, string prefix, bool mergeCoincident, out Dictionary<int, int> coincidentIndexMap)
        {
            ...
            Dictionary<Vector3, int> common = new Dictionary<Vector3, int>();
            int index = 0;
        ...
        public class Submesh
        {
            public Mesh mesh;
            public int index;
            public int[] m_Indexes => mesh.GetIndices(index);
            public MeshTopology m_Topology => MeshTopology.Triangles;
        }
        public class Model
        {
            ...
            public string name
            {
                get
                {
                    string meshName = mesh.name.Replace("(Clone)","").Trim(' ');
                    return string.IsNullOrEmpty(meshName) ? "default" : meshName;
                }
            }

            public int submeshCount => mesh.subMeshCount;
            public Matrix4x4 matrix => renderer ? renderer.localToWorldMatrix : Matrix4x4.identity;
            ...
            public Material[] materials
            {
                get
                {
                    Material[] result = renderer ? renderer.materials : new Material[0];
                    if (result.Length < submeshCount)
                    {
                        var defaultMat = new Material(Shader.Find("Standard"));
                        return result.Concat(Enumerable.Repeat(defaultMat, submeshCount - result.Length)).ToArray();
                    }
                    else
                        return result;
                }
            ...
            public Model(Renderer renderer) : this(renderer.GetMesh())
            {
